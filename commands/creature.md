...
Settable keys: `provider` (`auto`, `agents`, `claude`, `folder` or `vault`; `auto` is the default and picks `agents` when two or more coding agents are installed, else `claude`), `vault_root`, `xp_max`, `density` (`compact`, `minimal`, `full`, `sprite`, `ruler`), `sprite_height`, `border`, `columns`, `unicode`, `hidden`, `hookcard` (`changes`, `always` or `off`: when the one-line card shows on a host wired with `install --host codex|gemini|vibe|auggie|opencode|amp`; `changes` is the default and shows the first turn, any turn the session's xp or stage moved, and every tenth turn otherwise).
