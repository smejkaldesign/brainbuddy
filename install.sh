...
target_path_of() {
  local cmd="$1" cand
  # `bash ~/foo.sh` puts the script last, a bare path is the whole string, so
  # all three positions get tried. ~ expands by substitution rather than
  # `eval echo`, which would run command substitution out of the user's config.
...
write_shim() {
  # one prologue for both modes. it used to be duplicated per mode, which meant
  # a bug in the part that runs the host statusline had two places to live
  cat > "$SHIM" <<'EOF'
#!/bin/bash
# generated by the brainbuddy installer. safe to delete, reinstalling restores it.
# if we're already inside our own wrap, the thing we're about to run is us, and
# running it forks until the machine gives up. render nothing instead.
