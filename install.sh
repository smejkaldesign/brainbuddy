...
cat > "$SHIM" <<EOF
#!/bin/bash
# Generated by the brainbuddy installer. Safe to delete; run install.sh again to restore.
PYTHONPATH="$LIB" python3 -m brainbuddy.cli render 2>/dev/null || true
EOF
chmod +x "$SHIM"
