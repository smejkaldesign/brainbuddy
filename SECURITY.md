...
- anything that makes terminalcreature read the contents of a note. It counts files with
...
- anything that writes outside `~/.claude/terminalcreature/` and the statusline shim it
  installs, or that runs code it found in a note.

Expect a reply within a few days. Please include the version from `terminalcreature doctor`
